...
export default function SubjectInfoPage() {
  const { subject } = useTestState();
  const { setSubject, setPhase } = useTestActions();
      ...
      <div className="w-full max-w-md animate-fadeIn">
        <div className="panel p-8">
          <div className="text-center mb-8">
            ...
            <p className="text-base text-slate-500">被试信息登记</p>
          </div>
          ...
          <div className="space-y-5">
            <div>
              ...
              <input type="text" value={form.id} onChange={e => chg('id', e.target.value)}
                placeholder="如：S001"
                className={`ipt text-base ${errors.id ? 'border-red-300 bg-red-50' : ''}`} />
            ...
            <div>
              ...
              <input type="text" value={form.name} onChange={e => chg('name', e.target.value)}
                placeholder="请输入姓名"
                className={`ipt text-base ${errors.name ? 'border-red-300 bg-red-50' : ''}`} />
            ...
            <div>
              ...
              <input type="text" value={form.role} onChange={e => chg('role', e.target.value)}
                placeholder="如：心理学、人力资源管理"
                className={`ipt text-base ${errors.role ? 'border-red-300 bg-red-50' : ''}`} />
