/**
 * 导出数据结构对齐（任务五）。
 * 生成符合规范的 CSV（含 Form_Type、Page_Blur_Count、Bulk_Paste_Count 等字段）
 * 与完整 JSON Payload（供上传/备份）。
 */
import { runFullScoring, calculateModuleAScore, calculateModuleBScore } from '../utils/scoringEngine.js';
...
/**
 * 从 results 重建完整评分结果（results 只存了原始数据，评分需重算，
 * 但为保持与完成页一致，这里复用评分引擎）。
 */
...
export function buildCSV(results) {
  ...
  const row = [
    ...
    safe(results.formLabel || ('Form_' + (results.formType || 'A'))),
...
/**
 * 构建完整 JSON Payload（用于云端上传与 JSON 备份）。
...
export function buildPayload(results) {
  ...
  return {
    ...
    formLabel: results.formLabel || ('Form_' + (results.formType || 'A')),
